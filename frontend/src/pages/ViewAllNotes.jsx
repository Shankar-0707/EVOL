// src/pages/ViewAllNotes.jsx (UPDATED with Note Preview Modal + Soft Delete)
...
const ViewAllNotes = () => {
  ...
  const handleDelete = async (id, title) => {
    if (
      !window.confirm(`Are you sure you want to archive the note: "${title}"?`)
    ) {
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Failed to archive note.";
  ...
  if (isLoading) {
    ...
    content = (
      <div className="text-center py-16 text-gray-500 bg-white p-6 rounded-xl shadow-lg">
        <p className="text-lg">
          No active notes found yet! Time to write a new one.
        </p>
        <motion.button
    ...
    content = (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {notes.map((note, index) => (
          <motion.div
            key={note._id}
            className="bg-white p-6 rounded-2xl shadow-xl border-t-4 border-pink-400 flex flex-col hover:shadow-2xl transition duration-300 transform hover:scale-[1.01] cursor-pointer"
          ...
          >
            ...
            <div
              ...
              onClick={(e) => e.stopPropagation()} // prevent modal open on delete
            >
              <motion.button
...
// -----------------------------------------
// ✅ MODAL COMPONENT (FINAL FIXED CLOSE BUTTON + SCROLLABLE CONTENT)
// -----------------------------------------
const NotePreviewModal = ({ note, onClose }) => {
  ...
  return (
    ...
    >
      <motion.div
        className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 relative max-h-[80vh] overflow-y-auto"
      ...
      >
        <motion.button
          onClick={onClose}
          className="fixed top-4 right-4 text-gray-600 hover:text-pink-600 transition z-10 bg-white rounded-full p-1 shadow-md"
          whileHover={{ rotate: 90 }}
        >
          <XCircle size={24} />
        </motion.button>

        <h2 className="text-2xl font-bold text-pink-600 mb-2">{note.title}</h2>
        <p className="text-gray-500 mb-4">
          By <span className="font-semibold text-gray-800">{note.madeby}</span>{" "}
          • {formatDate(note.createdAt)}
        </p>
