...
const BASE_API_URL = 'http://localhost:5000/couple-quiz';
...
const ViewQuiz = () => {
    ...
    const renderContent = () => {
        ...
        if (isLoading) {
             return (/* ... Loading JSX ... */ <div className="text-center py-16 text-purple-500"><Loader2 size={32} className="mx-auto animate-spin mb-3" /><p className="text-lg font-medium">Loading quiz questions...</p></div>);
        }
        if (error) {
                    ...
                    {questions.map((q, index) => {
                        ...
                        >
                            <div className="flex justify-between items-start">
                                <div className="mb-4">
                                    <h3 className="text-sm font-medium text-gray-500">{q.category} Quiz</h3>
                                    <h2 className="text-2xl font-extrabold text-gray-800 flex items-start">
                                        <HelpCircle size={24} className="mr-3 text-purple-500 flex-shrink-0 mt-1"/>
                                        {q.question}
                                    </h2>
                                </div>
                            ...
                            >
                                <div className="mt-2 bg-green-50 p-4 rounded-lg border-l-4 border-green-400">
                                    ...
                                    <p className="text-base text-green-900 font-medium">"{q.correctAnswer}"</p>
                                    <p className="text-xs text-gray-400 mt-2">Generated on: {formatDate(q.createdAt)}</p>
                                </div>
                            </motion.div>
                            ...
                            <motion.button
                                ...
                                className={`mt-4 py-2 rounded-xl font-bold transition duration-200 shadow-sm 
                                    ${isRevealed ? 'bg-gray-400 text-white' : 'bg-pink-500 text-white hover:bg-pink-600'}`
                                }
    ...
    return (
        <div className="min-h-full relative overflow-hidden">
            ...
            <motion.div
                ...
                style={{
                    background: `radial-gradient(400px at ${mousePosition.x * 20 + 50}% ${mousePosition.y * 20 + 50}%, rgba(103, 58, 183, 0.15) 0%, rgba(255, 255, 255, 0) 100%)`,
