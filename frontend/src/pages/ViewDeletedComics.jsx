...
const ViewDeletedComics = () => {
  ...
  const handlePermanentDelete = async (id) => {
    if (
      !window.confirm(
        "WARNING: Are you absolutely sure you want to permanently delete this comic? This cannot be undone."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Failed to permanently delete comic.";
      ...
      return (
        <div className="text-center py-16 text-gray-500 bg-white p-6 rounded-xl shadow-lg">
          ...
          <motion.button
            ...
            className="mt-4 px-6 py-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition flex items-center justify-center gap-2 mx-auto"
            whileHover={{ scale: 1.05 }}
          >
            <RotateCcw size={20} />
            <span>Go Back to Comics</span>
          </motion.button>
        </div>
      );
    }
    ...
    return (
      <div className="space-y-8 max-w-5xl mx-auto">
        <p className="text-sm text-red-600 font-semibold flex items-center justify-center p-3 bg-red-50 border border-red-200 rounded-lg">
          <XCircle size={16} className="mr-2" /> These comics are archived.
          Click any card to review and permanently delete.
        </p>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-red-400">
                    Archived: {formatDate(comic.deletedAt)}
                  </p>
                </div>
